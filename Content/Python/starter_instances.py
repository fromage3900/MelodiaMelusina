"""Curated starter MI_* set for M_Master_Toon_Universal (10 showcase instances).

Each instance isolates ONE master capability (Nikki, flower shadow, nebula, magic, etc.).
Legacy MI_Universal_* under Instances/Environment are deprecated — not deleted; use
archive_unused_instances.py to move them to Instances/_Archive.

Imported by rebuild_material_instances.py, apply_starter_instances.py, and docs.
"""
from __future__ import annotations

import material_lib as lib

MASTER = f"{lib.MASTER_DIR}/M_Master_Toon_Universal.M_Master_Toon_Universal"
SHOWCASE_DIR = f"{lib.MATERIALS_ROOT}/Instances/Showcase"
LEGACY_ENV_DIR = lib.ENV_INST_DIR
ARCHIVE_ROOT = f"{lib.MATERIALS_ROOT}/Instances/_Archive"

# Canonical 10 starters — one capability each (see Docs/MATERIAL_INTEGRATION.md)
STARTER_INSTANCES: list[dict] = [
    {
        "name": "MI_Show_Default",
        "purpose": "Default showcase — neutral tint, full texture weight, zero stylization",
        "profile": "TP_Default",
        "vectors": {"BaseTint": (0.62, 0.58, 0.55, 1.0)},
        "scalars": {
            "TextureWeight": 1.0,
            "UVScale": 1.0,
            "Roughness": 0.65,
            "Metallic": 0.0,
            "NormalStrength": 1.0,
            "NormalPower": 1.0,
        },
        "key_params": "BaseTint, NormalStrength, TextureWeight (Palette + Parallax baseline)",
    },
    {
        "name": "MI_Show_StoneCliff",
        "purpose": "Stone surface — triplanar cliff with MF_ParallaxCore depth + normal power",
        "profile": "TP_Stone",
        "vectors": {"BaseTint": (0.44, 0.41, 0.38, 1.0)},
        "scalars": {
            "TextureWeight": 0.9,
            "Roughness": 0.85,
            "TriplanarTiling": 320.0,
            "ParallaxStrength": 0.55,
            "ParallaxScale": 0.06,
            "ParallaxSteps": 10.0,
            "ParallaxMode": 2.0,
            "ParallaxHeight": 0.88,
            "NormalStrength": 1.35,
            "NormalPower": 0.85,
            "MacroVariationStrength": 0.6,
            "MacroScale": 0.0006,
            "DetailStrength": 0.6,
            "DetailTiling": 12.0,
            "IttoCrackDepth": 0.55,
            "IttoWearAmount": 0.4,
            "IttoBreakupAmount": 0.2,
        },
        "switches": {"bTriplanar": True},
        "key_params": "ParallaxMode, IttoCrackDepth, NormalStrength (Parallax + Triplanar + Itto)",
    },
    {
        "name": "MI_Show_CherryBlossom",
        "purpose": "Flower shadow garden — projected petal shadows + sparkle on soft pink",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (1.0, 0.76, 0.85, 1.0),
            "ShadowFlowerColor": (0.92, 0.46, 0.72, 1.0),
            "SparkleColor": (1.0, 0.95, 0.85, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.3,
            "Roughness": 0.7,
            "PastelLift": 0.45,
            "ShadowFlowerStrength": 0.7,
            "ShadowFlowerScale": 5.0,
            "SparkleIntensity": 1.4,
            "SparkleScale": 11.0,
            "SparkleThreshold": 0.2,
            "SparkleContrast": 0.35,
            "SparkleTwinkleSpeed": 0.4,
        },
        "textures": {"SparkleMask": "twinkle", "FairyGlyphMask": "flower", "ShadowFlowerMask": "flower"},
        "switches": {"bSparkleAdvanced": True},
        "key_params": "ShadowFlowerStrength, SparkleThreshold (FlowerShadow + Nikki)",
    },
    {
        "name": "MI_Show_CelestialNebula",
        "purpose": "Nebula shading — MF_SpaceParallax constellation ramp + parallax nebula + galaxy",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.05, 0.06, 0.12, 1.0),
            "ConstellationRampLow": (0.02, 0.03, 0.10, 1.0),
            "ConstellationRampMid": (0.45, 0.22, 0.55, 1.0),
            "ConstellationRampHigh": (0.88, 0.76, 1.0, 1.0),
            "GlowColor": (0.7, 0.6, 1.0, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.15,
            "Roughness": 0.6,
            "ConstellationStrength": 0.85,
            "ConstellationScale": 1.8,
            "CelestialNebulaStrength": 0.95,
            "CelestialNebulaScale": 0.42,
            "CelestialGalaxyStrength": 0.5,
            "CelestialStarIntensity": 1.4,
            "CelestialToonSteps": 4.0,
            "GlowIntensity": 0.35,
            "InnerGlowIntensity": 0.2,
            "InnerGlowWidth": 1.3,
            "SparkleIntensity": 0.65,
            "SparkleThreshold": 0.12,
            "SparkleContrast": 0.4,
            "SparkleTwinkleSpeed": 0.55,
            "BloomBoost": 0.22,
        },
        "textures": {"StarMap": "twinkle", "SparkleMask": "twinkle"},
        "switches": {"bSparkleAdvanced": True},
        "key_params": "CelestialNebulaStrength, SparkleThreshold, InnerGlowWidth (Celestial + Nikki)",
    },
    {
        "name": "MI_Show_FairyHearts",
        "purpose": "Magic / fairy — heart motif, fairy dust, partial henshin wipe",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.9, 0.72, 0.88, 1.0),
            "FairyDustColor": (1.0, 0.92, 0.98, 1.0),
            "MotifColor": (1.0, 0.45, 0.72, 1.0),
            "MagicalPalette": (1.0, 0.72, 0.86, 1.0),
            "RimColor": (1.0, 0.88, 0.95, 1.0),
            "GlowColor": (0.95, 0.75, 0.92, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.3,
            "Roughness": 0.5,
            "PastelLift": 0.22,
            "RimIntensity": 0.18,
            "RimWidth": 1.4,
            "GlowIntensity": 0.12,
            "BloomBoost": 0.15,
            "FairyMotifStyle": 1.0,
            "FairyDustIntensity": 1.0,
            "FairyDustScale": 14.0,
            "FairyHighlightThreshold": 0.35,
            "MagicalTransform": 0.35,
            "MotifScale": 6.0,
            "TransformGlow": 3.0,
            "WipeSoftness": 0.25,
            "MadokaGlowAmount": 0.45,
            "MadokaVeinEmissive": 0.35,
        },
        "textures": {"FairyGlyphMask": "heart", "MotifMask": "heart", "SparkleMask": "sparkle4"},
        "key_params": "MagicalTransform, MadokaGlowAmount, FairyDustIntensity (Magical + Madoka + Nikki)",
    },
    {
        "name": "MI_Show_SkinSoft",
        "purpose": "Infinity Nikki environment look — soft pastel grading + rim + subtle glow (no skin controls)",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (1.0, 0.86, 0.80, 1.0),
            "DreamTint": (1.0, 0.92, 0.88, 1.0),
            "RimColor": (1.0, 0.88, 0.92, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.35,
            "Roughness": 0.42,
            "PastelLift": 0.18,
            "RimIntensity": 0.12,
            "RimWidth": 1.35,
            "RimBias": 0.05,
            "GlowIntensity": 0.06,
            "InnerGlowIntensity": 0.08,
            "InnerGlowWidth": 1.2,
            "DreamSaturation": 0.18,
            "DreamContrast": -0.08,
            "DreamShadowLift": 0.08,
            "DreamHighlightSoft": 0.05,
            "DreamHueShift": 0.02,
            "RimClamp": 0.92,
            "Iridescence": 0.08,
            "IridescenceRoughnessAtten": 0.65,
            "FabricSheen": 0.12,
        },
        "switches": {"bNikkiFast": True},
        "key_params": "PastelLift, bNikkiFast, DreamHueShift (Nikki soft)",
    },
    {
        "name": "MI_Show_NikkiHero",
        "purpose": "Infinity Nikki hero environment — full rim/glow/sparkle/iridescence stack",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.82, 0.72, 0.88, 1.0),
            "DreamTint": (0.95, 0.86, 1.0, 1.0),
            "RimColor": (0.85, 0.92, 1.0, 1.0),
            "GlowColor": (0.85, 0.75, 1.0, 1.0),
            "SparkleColor": (1.0, 0.95, 1.0, 1.0),
            "IridescenceTint": (0.7, 0.95, 1.0, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.35,
            "Roughness": 0.32,
            "PastelLift": 0.48,
            "DreamSaturation": 0.25,
            "DreamContrast": -0.12,
            "DreamShadowLift": 0.12,
            "DreamHighlightSoft": 0.08,
            "RimIntensity": 0.45,
            "RimSoftness": 0.45,
            "RimWidth": 1.65,
            "RimBias": 0.08,
            "GlowIntensity": 0.22,
            "InnerGlowIntensity": 0.25,
            "InnerGlowWidth": 1.25,
            "BloomBoost": 0.35,
            "Iridescence": 0.38,
            "IridescencePower": 1.35,
            "IridescenceBias": 0.05,
            "IridescenceRoughnessAtten": 0.75,
            "FabricSheen": 0.35,
            "SheenPower": 6.0,
            "SheenWidth": 1.25,
            "SheenBias": 0.02,
            "SparkleIntensity": 1.05,
            "SparkleThreshold": 0.18,
            "SparkleContrast": 0.45,
            "SparkleDriftSpeed": 0.15,
            "SparkleTwinkleSpeed": 0.75,
            "SparkleColorLerp": 0.35,
        },
        "textures": {"SparkleMask": "sparkle4"},
        "switches": {"bNikkiFast": False, "bNikkiHero": True, "bSparkleAdvanced": True, "bSheenUsesNormal": True},
        "key_params": "bNikkiHero, SparkleThreshold, IridescencePower (Nikki)",
    },
    {
        "name": "MI_Show_ForestFoliage",
        "purpose": "Foliage / forest floor — mossy green with dreamy shadow tint",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.28, 0.32, 0.22, 1.0),
            "ShadowDreamTint": (0.18, 0.22, 0.12, 1.0),
            "DreamTint": (0.35, 0.42, 0.28, 1.0),
            "MossColor": (0.25, 0.42, 0.2, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.9,
            "Roughness": 0.92,
            "PastelLift": 0.08,
            "DreamSaturation": 0.12,
            "DreamShadowLift": 0.1,
            "ShadowDreamStrength": 0.45,
            "ShadowSoftness": 0.75,
            "MossConcavityStrength": 0.55,
            "MossCurvatureSens": 2.2,
        },
        "key_params": "MossConcavityStrength, DreamSaturation (World + Nikki)",
    },
    {
        "name": "MI_Show_ContactRimHero",
        "purpose": "Cinematic — contact rim + atmospheric distance fade",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.5, 0.52, 0.58, 1.0),
            "ContactRimColor": (1.0, 0.95, 0.88, 1.0),
            "RimColor": (0.95, 0.92, 1.0, 1.0),
            "AtmosphericFadeColor": (0.62, 0.72, 0.82, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.7,
            "Roughness": 0.6,
            "ContactRimStrength": 1.0,
            "ContactRimPower": 5.0,
            "RimWidth": 1.55,
            "RimBias": 0.05,
            "RimClamp": 0.9,
            "RimIntensity": 0.12,
            "InnerGlowWidth": 1.15,
            "DistanceFadeStrength": 0.6,
            "DistanceFadeStart": 4500.0,
            "DistanceFadeEnd": 18000.0,
        },
        "key_params": "ContactRimStrength, RimWidth, DistanceFadeStrength (Cinematic + Nikki)",
    },
    {
        "name": "MI_Show_ElementHydro",
        "purpose": "Elemental hydro — wet iridescent glass with emissive boost",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.34, 0.52, 0.66, 1.0),
            "IridescenceTint": (0.8, 0.7, 0.95, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.2,
            "Roughness": 0.14,
            "Metallic": 0.0,
            "ElementType": 2.0,
            "ElementStrength": 0.8,
            "ElementEmissiveBoost": 0.4,
            "Iridescence": 0.3,
            "IridescencePower": 1.4,
            "IridescenceBias": 0.06,
            "IridescenceRoughnessAtten": 0.8,
            "FabricSheen": 0.25,
            "SheenPower": 5.5,
            "WetnessStrength": 0.6,
            "WetnessRoughness": 0.1,
        },
        "key_params": "ElementStrength, IridescencePower, WetnessStrength (Elemental + Nikki)",
    },
    {
        "name": "MI_Show_InkWash",
        "purpose": "Stylized ink wash — temporal boil + smear + wind",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.86, 0.84, 0.80, 1.0),
            "DreamTint": (0.92, 0.86, 0.98, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.55,
            "Roughness": 0.75,
            "DreamContrast": -0.1,
            "DreamHighlightSoft": 0.06,
            "DreamHueShift": 0.03,
            "TemporalStrength": 0.7,
            "BoilIntensity": 0.12,
            "SmearStrength": 0.14,
            "WindSpeed": 0.2,
            "NoiseScale": 1.8,
            "IttoInkStrength": 0.55,
            "IttoCrackDepth": 0.3,
            "IttoBreakupAmount": 0.18,
        },
        "textures": {"MotifMask": "bow"},
        "key_params": "TemporalStrength, IttoInkStrength, SmearStrength (Temporal + Itto + Nikki)",
    },
    {
        "name": "MI_Show_TrimsheetHero",
        "purpose": "ZenTrim trimsheet cross-demo — Base4K + CrackedToHell POM hero",
        "profile": "TP_Stone",
        "vectors": {"BaseTint": (0.58, 0.54, 0.50, 1.0)},
        "scalars": {
            "LayerBlend": 0.42,
            "LayerBlendMode": 1.0,
            "LayerHeightBias": 0.06,
            "LayerHeightSharpness": 5.5,
            "LayerBlendSoftness": 0.14,
            "LayerNormalBlend": 0.78,
            "LayerRoughnessBlend": 0.88,
            "ParallaxMode": 2.0,
            "ParallaxHeight": 0.9,
            "ParallaxStrength": 0.65,
            "NormalStrength": 1.3,
            "TextureWeight": 1.0,
        },
        "layer_a": "Base4K",
        "layer_b": "CrackedToHell",
        "key_params": "LayerBlend, ParallaxHeight, NormalStrength (Layers + Parallax)",
    },
    {
        "name": "MI_Show_ZenTorii",
        "purpose": "Zen torii cross-demo — vermillion tint + JRO motif masks",
        "profile": "TP_Default",
        "vectors": {
            "BaseTint": (0.78, 0.22, 0.18, 1.0),
            "MotifColor": (0.95, 0.35, 0.28, 1.0),
        },
        "scalars": {
            "TextureWeight": 0.65,
            "Roughness": 0.55,
            "MotifScale": 2.8,
            "RimIntensity": 0.18,
            "PastelLift": 0.12,
        },
        "textures": {"MotifMask": "zen05", "FairyGlyphMask": "zen03"},
        "key_params": "BaseTint, MotifScale, RimIntensity (Magical + Nikki)",
    },
    {
        "name": "MI_Show_AudioPulse",
        "purpose": "Audio MPC pulse — emissive + layer blend react (neutral until MPC driven)",
        "profile": "TP_Default",
        "vectors": {"BaseTint": (0.55, 0.48, 0.62, 1.0), "GlowColor": (0.85, 0.72, 1.0, 1.0)},
        "scalars": {
            "TextureWeight": 0.4,
            "Roughness": 0.7,
            "AudioReactivity": 0.85,
            "BassWeight": 1.0,
            "MidWeight": 0.6,
            "TrebleWeight": 0.35,
            "AudioEmissiveStrength": 0.45,
            "AudioLayerBlendPulse": 0.12,
            "AudioRoughnessPulse": 0.08,
            "GlowIntensity": 0.2,
        },
        "key_params": "AudioReactivity, AudioEmissiveStrength (Audio)",
    },
]

STARTER_NAMES: frozenset[str] = frozenset(s["name"] for s in STARTER_INSTANCES)

# Legacy MI_Universal_* names superseded by starters (for archive / migration docs)
LEGACY_ALIASES: dict[str, str] = {
    "MI_Universal_Default": "MI_Show_Default",
    "MI_Universal_LayeredStone": "MI_Show_StoneCliff",
    "MI_Universal_DreamyPastel": "MI_Show_SkinSoft",
    "MI_Universal_CherryBlossom": "MI_Show_CherryBlossom",
    "MI_Universal_CelestialNebula": "MI_Show_CelestialNebula",
    "MI_Universal_FairyHearts": "MI_Show_FairyHearts",
    "MI_Universal_ForestFloor": "MI_Show_ForestFoliage",
    "MI_Universal_ContactRimHero": "MI_Show_ContactRimHero",
    "MI_Universal_ElementHydro": "MI_Show_ElementHydro",
    "MI_Universal_TemporalInk": "MI_Show_InkWash",
    "MI_Universal_NikkiHero": "MI_Show_NikkiHero",
}
