...
try:
    ...
    meso_plan = plans.spawn_village_plan(location=(115, 0, 0))
    s._active_style_genome = os_genome.load_genome("meso_pyramid_courtyard_v1")
    mroot, mmsg = compose.compose_world(s, bpy.context, meso_plan, "MESOAMERICAN_PYRAMID", 0.85, "COLLECTION")
    mesoamerican_export_root = mroot
    s._active_style_genome = None
    print(f"  mesoamerican_compose: {mmsg} metrics={verify_hooks.compose_metrics(mroot)}")
    if mroot.get("surreal_style_genome_id") != "meso_pyramid_courtyard_v1":
        print(f"  !! FAIL: mesoamerican genome stamp got {mroot.get('surreal_style_genome_id')}")
        all_ok = False
    else:
        print("  mesoamerican_compose: OK")
except Exception as e:
    print(f"  mesoamerican compose error: {e}")
    all_ok = False
...
try:
    ...
    if mesoamerican_export_root is not None:
        mm = export.build_world_manifest(mesoamerican_export_root, monolith=s)
        ...
        if msg.get("family") != "Mesoamerican":
            raise RuntimeError(f"mesoamerican family mismatch: {msg.get('family')}")
        ...
        if msg.get("resolved_compose_roles", {}).get("corner_tower") != "_lib_PILLAR":
            raise RuntimeError("MESOAMERICAN_PYRAMID corner_tower must be pillar (no tower)")
        if msg.get("resolved_compose_roles", {}).get("monument") != "_lib_GREYBOX_STAIR_BLOCK":
            raise RuntimeError("MESOAMERICAN_PYRAMID monument role mismatch")
        if msg.get("resolved_compose_roles", {}).get("sacred") != "_lib_PUBLIC_FOUNTAIN":
            raise RuntimeError("MESOAMERICAN_PYRAMID sacred role mismatch")
        print("  mesoamerican manifest embed: OK")
