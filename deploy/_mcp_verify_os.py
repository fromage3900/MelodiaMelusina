...
try:
    ...
    if len(bar_objs) < 3:
        ...
        all_ok = False
    beaux_spec = GRAPH_REGISTRY["BEAUX_ARTS_PLAZA"]["spec"]
    beaux_objs = spawn_graph(bpy.context, s, beaux_spec[:3], spacing=11.0, graph_id="BEAUX_ARTS_PLAZA")
    print(f"  spawn_graph BEAUX_ARTS_PLAZA partial: {len(beaux_objs)} objects")
    if len(beaux_objs) < 2:
        print(f"  !! FAIL: BEAUX_ARTS_PLAZA spawn got {len(beaux_objs)}")
        all_ok = False
    banned = {"TOWER", "TESSELLATION_TOWER", "BELL_TOWER", "WATCHTOWER", "OBELISK", "KEEP"}
    beaux_types = {at for at, _ in beaux_spec}
    if banned & beaux_types:
        print(f"  !! FAIL: BEAUX_ARTS_PLAZA banned arch types: {banned & beaux_types}")
        all_ok = False
    else:
        print("  BEAUX_ARTS_PLAZA tower-ban: OK")
    ...
    rp_beaux = run_research_preset(bpy.context, "beaux_arts_plaza_graph", monolith=s)
    if rp_beaux.get("mode") != "graph" or rp_beaux.get("count", 0) < 4:
        print(f"  !! FAIL: beaux_arts_plaza_graph spawn: {rp_beaux}")
        all_ok = False
    else:
        print(f"  research_preset beaux_arts_plaza_graph: {rp_beaux['count']} modules")
...
if not xf:
    ...
    all_ok = False
elif "BEAUX_ARTS_PLAZA" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing BEAUX_ARTS_PLAZA")
    all_ok = False
elif "ZEN_STREAM_GARDEN" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing ZEN_STREAM_GARDEN")
    all_ok = False
else:
    print(f"  axis_compression type={xf.get('type')} BRUTALIST_PLAZA+BEAUX_ARTS_PLAZA: OK")
...
ba = os_genome.load_genome("beaux_arts_plaza_v1")
if ba.get("grammar_id") != "BEAUX_ARTS_PLAZA" or ba.get("compose_style") != "BEAUX_ARTS_PLAZA":
    print(f"  !! FAIL: beaux_arts_plaza_v1")
    all_ok = False
elif os_genome.genome_family(ba) != "BeauxArts":
    print(f"  !! FAIL: beaux_arts family={os_genome.genome_family(ba)}")
    all_ok = False
elif ba.get("surreal_transform") != "axis_compression":
    print(f"  !! FAIL: beaux_arts transform={ba.get('surreal_transform')}")
    all_ok = False
elif os_styles.get("BEAUX_ARTS_PLAZA", {}).get("corner_tower") != "_lib_PILLAR":
    print("  !! FAIL: BEAUX_ARTS_PLAZA corner_tower not PILLAR")
    all_ok = False
elif ba.get("compose_roles", {}).get("corner_tower") != "_lib_PILLAR":
    print(f"  !! FAIL: beaux_arts genome corner_tower={ba.get('compose_roles')}")
    all_ok = False
elif os_styles.get("BEAUX_ARTS_PLAZA", {}).get("gate") != "_lib_ARCHWAY_ADV":
    print("  !! FAIL: BEAUX_ARTS_PLAZA compose_roles missing")
    all_ok = False
else:
    print("  beaux_arts_plaza_v1 + BEAUX_ARTS_PLAZA compose_roles: OK")
