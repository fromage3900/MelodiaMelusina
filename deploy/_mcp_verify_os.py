...
if ad.get("compose_style") != "ART_DECO" or os_genome.genome_family(ad) != "ArtDeco":
    ...
    all_ok = False
elif os_styles.get("ART_DECO", {}).get("monument") != "_lib_GREYBOX_RAMP":
    print("  !! FAIL: ART_DECO monument must be ramp (no obelisk)")
    all_ok = False
elif ad.get("compose_roles", {}).get("corner_tower") != "_lib_PILLAR":
    print(f"  !! FAIL: art_deco_lobby_v1 compose_roles corner_tower={ad.get('compose_roles')}")
    all_ok = False
else:
    banned = {"TOWER", "TESSELLATION_TOWER", "BELL_TOWER", "WATCHTOWER", "OBELISK", "KEEP"}
    deco_types = {at for at, _ in GRAPH_REGISTRY["ART_DECO"]["spec"]}
    if deco_types & banned:
        print(f"  !! FAIL: ART_DECO grammar still has banned tower types: {deco_types & banned}")
        all_ok = False
    else:
