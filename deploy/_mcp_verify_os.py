...
try:
    ...
    if len(deco_objs) < 2:
        ...
        all_ok = False
    ball_spec = GRAPH_REGISTRY["MESOAMERICAN_BALLCOURT"]["spec"]
    ball_objs = spawn_graph(bpy.context, s, ball_spec[:3], spacing=11.0, graph_id="MESOAMERICAN_BALLCOURT")
    print(f"  spawn_graph MESOAMERICAN_BALLCOURT partial: {len(ball_objs)} objects")
    if len(ball_objs) < 2:
        print(f"  !! FAIL: MESOAMERICAN_BALLCOURT spawn got {len(ball_objs)}")
        all_ok = False
...
if not xf:
    ...
    all_ok = False
elif "MESOAMERICAN_BALLCOURT" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing MESOAMERICAN_BALLCOURT")
    all_ok = False
elif "ZEN_STREAM_GARDEN" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing ZEN_STREAM_GARDEN")
    all_ok = False
else:
    print(f"  axis_compression type={xf.get('type')} BRUTALIST_PLAZA+MESOAMERICAN_BALLCOURT: OK")
...
mb = os_genome.load_genome("meso_ballcourt_v1")
if mb.get("compose_style") != "MESOAMERICAN_BALLCOURT" or os_genome.genome_family(mb) != "Mesoamerican":
    print(f"  !! FAIL: meso_ballcourt_v1 compose/family")
    all_ok = False
elif mb.get("grammar_id") != "MESOAMERICAN_BALLCOURT":
    print(f"  !! FAIL: meso_ballcourt_v1 grammar={mb.get('grammar_id')}")
    all_ok = False
elif mb.get("surreal_transform") != "axis_compression":
    print(f"  !! FAIL: meso_ballcourt_v1 transform={mb.get('surreal_transform')}")
    all_ok = False
elif os_styles.get("MESOAMERICAN_BALLCOURT", {}).get("corner_tower") != "_lib_PILLAR":
    print("  !! FAIL: MESOAMERICAN_BALLCOURT compose_roles missing / tower banned")
    all_ok = False
elif os_styles.get("MESOAMERICAN_BALLCOURT", {}).get("gate") != "_lib_ARCHWAY_ADV":
    print("  !! FAIL: MESOAMERICAN_BALLCOURT gate role missing")
    all_ok = False
else:
    print("  meso_ballcourt_v1 + MESOAMERICAN_BALLCOURT compose_roles: OK")
