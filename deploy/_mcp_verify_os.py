...
if not getattr(s, "_surreal_patched", False):
    # patch may set this; tolerate if kits still dispatch
    if not getattr(s, "_KIT_DISPATCH", None):
...
try:
    ...
    if len(deco_objs) < 2:
        ...
        all_ok = False
    greek_spec = GRAPH_REGISTRY["GREEK_STOA"]["spec"]
    greek_objs = spawn_graph(bpy.context, s, greek_spec[:3], spacing=11.0, graph_id="GREEK_STOA")
    print(f"  spawn_graph GREEK_STOA partial: {len(greek_objs)} objects")
    if len(greek_objs) < 2:
        print(f"  !! FAIL: GREEK_STOA spawn got {len(greek_objs)}")
        all_ok = False
...
if not xf:
    ...
    all_ok = False
elif "GREEK_STOA" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing GREEK_STOA")
    all_ok = False
else:
    print(f"  axis_compression type={xf.get('type')} BRUTALIST_PLAZA+GREEK_STOA: OK")
...
gs = os_genome.load_genome("greek_stoa_v1")
...
greek_types = {
    ...
    for row in (GRAPH_REGISTRY.get("GREEK_STOA", {}).get("spec") or [])
}
if gs.get("compose_style") != "GREEK_STOA" or os_genome.genome_family(gs) != "Greek":
    print(f"  !! FAIL: greek_stoa_v1 compose/family")
    all_ok = False
elif gs.get("grammar_id") != "GREEK_STOA":
    print(f"  !! FAIL: greek_stoa_v1 grammar={gs.get('grammar_id')}")
    all_ok = False
elif gs.get("surreal_transform") != "axis_compression":
    print(f"  !! FAIL: greek_stoa_v1 transform={gs.get('surreal_transform')}")
    all_ok = False
elif os_styles.get("GREEK_STOA", {}).get("gate") != "_lib_ARCHWAY_ADV":
    print("  !! FAIL: GREEK_STOA compose_roles missing")
    all_ok = False
elif os_styles.get("GREEK_STOA", {}).get("corner_tower") != "_lib_PILLAR":
    print("  !! FAIL: GREEK_STOA corner_tower must be pillar (tower ban)")
    all_ok = False
elif banned & greek_types:
    print(f"  !! FAIL: GREEK_STOA grammar has banned arch types: {banned & greek_types}")
    all_ok = False
else:
    print("  greek_stoa_v1 + GREEK_STOA compose_roles: OK")
