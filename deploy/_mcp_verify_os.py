...
if meso.get("compose_style") != "MESOAMERICAN_PYRAMID" or os_genome.genome_family(meso) != "Mesoamerican":
    ...
    print("  !! FAIL: MESOAMERICAN_PYRAMID corner_tower must be pillar (no tower spines)")
    all_ok = False
elif os_styles.get("MESOAMERICAN_PYRAMID", {}).get("monument") != "_lib_GREYBOX_STAIR_BLOCK":
    print("  !! FAIL: MESOAMERICAN_PYRAMID monument must be stair block")
    all_ok = False
else:
    banned = {"TOWER", "TESSELLATION_TOWER", "BELL_TOWER", "WATCHTOWER", "OBELISK", "KEEP"}
    meso_types = {at for at, _ in GRAPH_REGISTRY["MESOAMERICAN_PYRAMID"]["spec"]}
    hit = meso_types & banned
    if hit:
