...
try:
    ...
    if len(deco_objs) < 2:
        ...
        all_ok = False
    deco_ind_spec = GRAPH_REGISTRY["ART_DECO_INDUSTRIAL"]["spec"]
    deco_ind_objs = spawn_graph(bpy.context, s, deco_ind_spec[:3], spacing=11.0, graph_id="ART_DECO_INDUSTRIAL")
    print(f"  spawn_graph ART_DECO_INDUSTRIAL partial: {len(deco_ind_objs)} objects")
    if len(deco_ind_objs) < 2:
        print(f"  !! FAIL: ART_DECO_INDUSTRIAL spawn got {len(deco_ind_objs)}")
        all_ok = False
...
if not xf:
    ...
    all_ok = False
elif "ART_DECO_INDUSTRIAL" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing ART_DECO_INDUSTRIAL")
    all_ok = False
elif "ZEN_STREAM_GARDEN" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing ZEN_STREAM_GARDEN")
    all_ok = False
else:
    print(f"  axis_compression type={xf.get('type')} BRUTALIST_PLAZA+ART_DECO_INDUSTRIAL: OK")
...
adi = os_genome.load_genome("art_deco_industrial_v1")
if adi.get("compose_style") != "ART_DECO_INDUSTRIAL" or os_genome.genome_family(adi) != "ArtDeco":
    print(f"  !! FAIL: art_deco_industrial_v1 compose/family")
    all_ok = False
elif adi.get("grammar_id") != "ART_DECO_INDUSTRIAL":
    print(f"  !! FAIL: art_deco_industrial_v1 grammar={adi.get('grammar_id')}")
    all_ok = False
elif adi.get("surreal_transform") != "axis_compression":
    print(f"  !! FAIL: art_deco_industrial_v1 transform={adi.get('surreal_transform')}")
    all_ok = False
elif os_styles.get("ART_DECO_INDUSTRIAL", {}).get("corner_tower") != "_lib_PILLAR":
    print("  !! FAIL: ART_DECO_INDUSTRIAL compose_roles missing / tower banned")
    all_ok = False
elif os_styles.get("ART_DECO_INDUSTRIAL", {}).get("gate") != "_lib_CUSPED_ARCH":
    print("  !! FAIL: ART_DECO_INDUSTRIAL gate role missing")
    all_ok = False
else:
    print("  art_deco_industrial_v1 + ART_DECO_INDUSTRIAL compose_roles: OK")
