...
if meso.get("compose_style") != "MESOAMERICAN_PYRAMID" or os_genome.genome_family(meso) != "Mesoamerican":
    ...
    print("  !! FAIL: MESOAMERICAN_PYRAMID corner_tower must be pillar (no towers)")
    all_ok = False
else:
    banned = {"TOWER", "TESSELLATION_TOWER", "BELL_TOWER", "WATCHTOWER", "OBELISK", "KEEP"}
    meso_spec_types = {t[0] for t in load_grammar_graph("MESOAMERICAN_PYRAMID")}
    meso_reg = {t[0] for t in GRAPH_REGISTRY["MESOAMERICAN_PYRAMID"]["spec"]}
    hit = banned.intersection(meso_spec_types | meso_reg)
    if hit:
        print(f"  !! FAIL: MESOAMERICAN_PYRAMID banned tower types: {hit}")
        all_ok = False
    else:
