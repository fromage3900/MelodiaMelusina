...
if not xf2:
    ...
    print("  !! FAIL: vertical_stretch still lists ART_DECO (tower-ban rematerialize)")
    all_ok = False
else:
...
if ad.get("compose_style") != "ART_DECO" or os_genome.genome_family(ad) != "ArtDeco":
    ...
    all_ok = False
elif (ad.get("compose_roles") or {}).get("corner_tower") != "_lib_PILLAR":
    print(f"  !! FAIL: art_deco genome corner_tower override={(ad.get('compose_roles') or {}).get('corner_tower')}")
    all_ok = False
else:
    ...
    if banned & deco_types:
        print(f"  !! FAIL: ART_DECO grammar has banned arch types: {banned & deco_types}")
        all_ok = False
    else:
        print("  art_deco_lobby_v1 + ART_DECO compose_roles (tower-ban): OK")
