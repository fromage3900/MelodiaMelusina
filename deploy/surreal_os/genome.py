...
def resolve_genome_manifest(monolith, style_key: str, genome_id: str | None = None) -> dict:
    ...
    if not gid and style_key == "BALINESE_PURA_COURTYARD":
        gid = "balinese_pura_courtyard_v1"
    if not gid and style_key == "BRUTALIST_PLAZA":
        gid = "brutalist_plaza_v1"
    if gid:
        try:
