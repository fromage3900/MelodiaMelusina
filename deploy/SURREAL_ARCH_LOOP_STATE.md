...
- **`meso_pyramid_courtyard_v1`** genome + **`MESOAMERICAN_PYRAMID`** grammar (retaining tiers, ceremonial stair, arcade, portal, sacred pool)
- **`MESOAMERICAN_PYRAMID`** compose style — corner role = pillar (no tower kits); library bake for retaining wall + stair block
- Research study `research/mesoamerican/01_pyramid_courtyard.md` + graph preset; `axis_compression` transform
