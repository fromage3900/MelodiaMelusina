...
- **`meso_pyramid_courtyard_v1`** genome + **`MESOAMERICAN_PYRAMID`** grammar (retaining terrace, ramp, ceremonial stair, colonnade, portal, sacred pool)
- **`MESOAMERICAN_PYRAMID`** compose style — pillar corner markers (no tower spines); `axis_compression`
- Research study `research/mesoamerican/01_pyramid_courtyard.md` + curated graph preset
- Library bake for `RETAINING_WALL` / `GREYBOX_STAIR_BLOCK` / `GREYBOX_RAMP`; world manifest embed verify; catalog **31**
