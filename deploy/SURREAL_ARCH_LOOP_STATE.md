...
- **`meso_pyramid_courtyard_v1`** genome + **`MESOAMERICAN_PYRAMID`** grammar (terraces, stair, arcade, portal, pool)
- **`MESOAMERICAN_PYRAMID`** compose style — pillar corner role, stair monument, fountain sacred (no tower spines)
- Library bake for **`RETAINING_WALL`** + **`GREYBOX_STAIR_BLOCK`**; `axis_compression` transform
- Research study `research/mesoamerican/01_pyramid_courtyard.md` + curated graph preset
- World/OS verify tiers; genome catalog **31**
...
- Streamline Moderne civic facade / arcade chain (horizontal, no tower)
...
- See top of file — horizontal civic chain; banned tower types excluded from grammar + compose roles
