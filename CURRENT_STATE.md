...
**Crash culprit isolated: `PCG_BaroqueAtriumEx`.** Static property inspection via Python reflection on `PCGExCreateShapesSettings` didn't work cleanly (most properties are C++-protected, `dir()`-based enumeration throws on protected getters) — with the `-unattended` boot fix making relaunches fast and reliable, it was more efficient to just test the 4 risky graphs one at a time. `AtriumEx` crashed the engine on generate exactly as predicted (editor went unresponsive, confirmed dead via `monolith_status` failing). Recovered via `-unattended` relaunch in under a minute. `ColonnadeEx`/`FacadeEx`/`RotundaEx` (same node pattern) not yet tested — still unknown if they share the same bug or are fine; test them one at a time before assuming either way. Do not re-generate `AtriumEx` until its `PCGExCreateShapesSettings` node is fixed or replaced (vanilla `PCGCreatePointsSettings`, proven Escher `*Ex` pattern).
